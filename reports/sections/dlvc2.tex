...
\subsection{Softmax hyperparameter optiomization}
...
We used a grid search for finding good parameters for weight decay and learning rate. Using very extreme values (see\ref{figparam-opt-hog6} we were able to show that most combinations yield in good results, but that a low learning rate and a high weight decay lead to a model with far too less training and thus bad results. Our first test were from that range since the assignment depicts them as good values. But in general, it is fair to say that HOG features instead of the raw images lead to significantly higher validation accuracy. Classifier on raw images primarily overfits (increasing training accuracy but sinking validation accuracy).
...
\subsection{Classify new image}
We also implememted the manual image identification task. The provided image is classified as a frog with high confidence (99.8\%). To make sure this is not a problem with the model, we tested several images. The image '\textbf{cat\_new.jpg}', which is also provided, depicts a cat and is identified with 68.1\% confidence, thus the setup it sound. 
