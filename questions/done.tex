...
Until the point of optimal capacity decreasing the training error results in a better test error. From then on the test error increases again, due to overfitting. The difference between training and test error is the generalization gap that results from tailoring the model too strongly to the training/validation data.
...
For better imagination, think of a rugged terrain, whithout sight. To get to the lowset/highest point, one will just follow the steepest slope. 

IN ML setting, the algorithm will compute the current gradients $\Theta'$ using the derivative and then go a certain amount $\alpha*||\Theta'||$ into that direction. The hyperparameter $\alpha$ is called the learning rate.
\subsection{What is the gradient of a function? }
A function f, that is differentiable, can have partial derivatives for all input parameters so that $F_{x_i}(x)$ encodes how fast f changes with argument $x_i$ at pouint x.  The gradient ist a vector of all these partial derivatives. It point in the direction of greatest increase. If the value is 0, L is flat and we have to stop (local/global minimum).
...
The bias is less critical and can be set to for initializetion.

the weights are critical because of the multiplication and the gradient descent converges very different depending on the initialization. Extremely important is a difference in the weight matrix to break symmetry. A good  initialization is $Norm(0,1)\gamma$, the standard normal distribution times a hyper parameter $\gamma$ that control the magnitude of the weights.
\subsection{What happens if the weights are set too large or too small?}
The hyper parameter $\gamma$ control the magnitude of gradient and step size, so if set too large, the activation explodes, and if to small, the activation vanishes. In both cases, the algorithm most likely won't converge on a good model. 
...
The so called Xavier initialization measures the variance of the input D (usually1) and sets $\gamma$ in a way that it preserves this variance: $\gamma= 1/\sqrt{D}$. It is very popular in DL.
...
\begin{minipage}{0.4\textwidth}
\includegraphics[width=0.96\textwidth]{./img/ffnn1.jpeg}
\includegraphics[width=0.96\textwidth]{./img/ffnn2.jpeg}
\end{minipage}
\begin{minipage}{0.6\textwidth}
\begin{itemize}
